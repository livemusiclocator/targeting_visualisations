...
# Create a base figure for the bubble chart
fig = px.scatter(
    filtered_lga_stats, 
    x='Venue Count',
    y='Avg Frequency',
...
# Add dropdown for LGA selection
...
for lga in sorted_lgas:
    # For each LGA, create a button that highlights that LGA
    # and dims all others
    opacities = [0.7 if row_lga == lga else 0.2 for row_lga in filtered_lga_stats['LGA']]
    buttons.append(dict(
        method='update',
        label=lga,
...
avg_freq = filtered_lga_stats['Avg Frequency'].mean()
fig.add_shape(
    type="line",
    x0=0,
    y0=avg_freq,
    x1=filtered_lga_stats['Venue Count'].max() * 1.05,
    y1=avg_freq,
    line=dict(
        color="red",
        width=1,
...
fig.add_annotation(
    x=filtered_lga_stats['Venue Count'].max() * 0.98,
    y=avg_freq * 1.05,
...
output_file = 'lga_venues_frequency_interactive_with_dropdown.html'
fig.write_html(output_file)
print(f"Interactive chart with dropdown saved as {output_file}")

# Also save as a static image for reference
fig.write_image('lga_venues_frequency_interactive_with_dropdown.png', scale=2)
print("Static version saved as lga_venues_frequency_interactive_with_dropdown.png")
...
print("\nTo view the interactive chart with LGA selection dropdown, open the HTML file in a web browser.")